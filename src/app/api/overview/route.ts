...
function generateSessionId(ip: string, userAgent: string): string {
  ...
  try {
    const hash = btoa(`${ip}-${userAgent}-${timestamp}`)
      .replace(/[^a-zA-Z0-9]/g, "")
      .substring(0, 32);
    return hash;
  } catch {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check for spam prevention - same IP and page in last minute
    const oneMinuteAgo = new Date(Date.now() - 60 * 1000);
    const recentVisit = await prisma.visitorTracking.findFirst({
      where: {
        ipAddress,
        pageUrl: page_url,
    ...
    const visitor = await prisma.visitorTracking.create({
      data: {
        ipAddress,
        location: locationData ? JSON.parse(JSON.stringify(locationData)) : null,
...
// GET - Fetch visitor statistics (admin)
export async function GET(request: NextRequest) {
  try {
    ...
    const totalVisits = await prisma.visitorTracking.count({
      where: {
        createdAt: { gte: startDate },
      },
    });

    const uniqueVisitors = await prisma.visitorTracking.groupBy({
      by: ["ipAddress"],
      where: {
        createdAt: { gte: startDate },
      },
